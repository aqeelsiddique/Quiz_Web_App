  ...
  async.parallel(
    ...
    function (err, results) {
      ...
      for (
        ...
        for (
          let process_g_iter = 0;
          process_g_iter < results.process.category.length;
          process_g_iter++
        ) {
          if (
            results.categories[all_g_iter]._id.toString() ==
            results.process.category[process_g_iter]._id.toString()
          ) {
...
exports.process_update_post = [
  ...
  body('machine').trim().escape(),
  body('question').trim().escape(),
  body('option1').trim().escape(),
  body('option2').trim().escape(),
  ...
  (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      Process.findByIdAndUpdate(req.params.id, process, {}, function (
        err,
        theprocess
      ) {
        if (err) {
