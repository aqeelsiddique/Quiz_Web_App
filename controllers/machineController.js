...
exports.machine_list = (req, res, next) => {
  Machine.find()
    .sort([['name', 'ascending']])
    .exec((err, list_machines) => {
      if (err) {
        return next(err);
      }
  ...
  async.parallel(
    {
      ...
      machines_processes: callback => {
        Process.find({ machine: req.params.id }, 'name question option1 option2').exec(callback);
      },
    },
    (err, results) => {
      if (err) {
...
exports.machine_delete_get = (req, res, next) => {
  async.parallel(
    {
      machine: callback => {
        Machine.findById(req.params.id).exec(callback);
...
exports.machine_delete_post = (req, res, next) => {
  async.parallel(
    {
      machine: callback => {
    ...
    (err, results) => {
      ...
      if (results.machines_processes.length > 0) {
        ...
        Machine.findByIdAndRemove(req.body.machineid, err => {
          if (err) {
            return next(err);
          }
...
exports.machine_update_get = (req, res, next) => {
...
exports.machine_update_post = [
  ...
  (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      Machine.findByIdAndUpdate(req.params.id, machine, {}, (err, themachine) => {
        if (err) {
          return next(err);
        }
