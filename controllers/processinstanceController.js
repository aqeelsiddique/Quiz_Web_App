...
exports.processinstance_list = function (req, res, next) {
  ProcessInstance.find()
    ...
    .exec(function (err, processinstances) {
      ...
      res.render('processinstance_list', {
  ...
  ProcessInstance.findById(req.params.id)
    ...
    .exec(function (err, processinstance) {
      ...
      if (processinstance == null) {
        // No results.
        const err = new Error('Process copy not found');
        err.status = 404;
        return next(err);
      }
      // Successful, so render.
      res.render('processinstance_detail', {
        title: 'Process:',
...
exports.processinstance_create_get = function (req, res, next) {
  Process.find({}, 'name').exec(function (err, processes) {
    ...
    // Successful, so render.
    res.render('processinstance_form', {
      title: 'Create ProcessInstance',
...
exports.processinstance_create_post = [
  ...
  body('scheduled_end', 'Invalid date')
    .optional({ checkFalsy: true })
    .isISO8601(),

  // Sanitize fields.
  body('process').trim().escape(),
  body('operator').trim().escape(),
  body('status').trim().escape(),
  body('scheduled_end').toDate(),
...
exports.processinstance_delete_get = function (req, res, next) {
  ProcessInstance.findById(req.params.id)
    ...
    .exec(function (err, processinstance) {
      ...
      // Successful, so render.
      res.render('processinstance_delete', {
        title: 'Delete ProcessInstance',
        processinstance: processinstance,
      });
    });
};
...
exports.processinstance_delete_post = function (req, res, next) {
  ...
  ProcessInstance.findByIdAndRemove(req.body.id, function deleteProcessInstance(
    err
  ) {
    if (err) {
      return next(err);
...
exports.processinstance_update_get = function (req, res, next) {
  ...
  async.parallel(
    ...
    function (err, results) {
      ...
      if (results.processinstance == null) {
        // No results.
        const err = new Error('Process copy not found');
        err.status = 404;
        return next(err);
...
exports.processinstance_update_post = [
  ...
  (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      // Data from form is valid.
      ProcessInstance.findByIdAndUpdate(
        req.params.id,
        processinstance,
        {},
        function (err, theprocessinstance) {
          if (err) {
            return next(err);
          }
